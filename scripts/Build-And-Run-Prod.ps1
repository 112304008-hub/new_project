...
${composeProd} = Join-Path (Join-Path $PSScriptRoot '..') 'infra/compose/docker-compose.prod.yml'
${composeOverride} = Join-Path (Join-Path $PSScriptRoot '..') 'infra/compose/docker-compose.override.yml'
${composeGenerated} = Join-Path (Join-Path $PSScriptRoot '..') 'infra/compose/docker-compose.override.local.generated.yml'
...
$useGenerated = $false
try { $test = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 18080); $test.Start(); $test.Stop() } catch { $useGenerated = $true }
...
if ($useGenerated) {
  ...
  @(
    ...
    "      - ${freePort}:80",
    "    volumes:",
    "      - ../caddy/conf/Caddyfile.local:/etc/caddy/Caddyfile:ro"
...
if (-not $domain) {
  ...
  if (Test-Path $envFile) { foreach ($line in Get-Content $envFile) { if ($line -match '^\s*DOMAIN\s*=\s*(.+)$') { $domain = $matches[1].Trim(); break } } }
}
...
for ($i = 1; $i -le $maxAttempts; $i++) {
  try {
    ...
    else { Write-Host "Attempt #${i}: HTTP $($resp.StatusCode) — retrying..." }
  }
  catch { Write-Host "Attempt #${i}: waiting for Caddy/web to be ready..." }
  Start-Sleep -Seconds 3
}
...
if (-not $ok) {
  ...
  try {
    ...
    if ($LASTEXITCODE -eq 0) { Write-Host "Internal health OK. App is running." -ForegroundColor Yellow; exit 0 }
  } catch {}
  exit 1
}
